...
import StyledPathfindersMyCustomDxLibParagraphParamWrapper from './styles';

// interface for props
interface PathfindersMyCustomDxLibParagraphParamProps extends PConnFieldProps {
...
function PathfindersMyCustomDxLibParagraphParam(props: PathfindersMyCustomDxLibParagraphParamProps) {
  const {
    getPConnect,
    value,
    hideLabel = false,
 ...
 // BUG-547602: Temporary type coercion for 8.5 until DXAPIs are enhanced to pass original pxViewMetadata JSON, respecting boolean primitives
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'DISPLAY_ONLY') {
    ...
    return displayMode === 'DISPLAY_ONLY' ? (
      <StyledPathfindersMyCustomDxLibParagraphParamWrapper>
      {displayComp}
      </StyledPathfindersMyCustomDxLibParagraphParamWrapper>
    ) : (
      <StyledPathfindersMyCustomDxLibParagraphParamWrapper>
...
export default withConfiguration(PathfindersMyCustomDxLibParagraphParam);
